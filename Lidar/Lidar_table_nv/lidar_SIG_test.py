...
class LidarKit:
    def __init__(self, port="/dev/ttyS0", baudrate=230400, debug=False):
        ...
        self.ser = None
        self.running = False
        self.reader_thread = None  # Nom explicite pour le thread
    ...
    def start(self):
        ...
        self.open()
        self._set_pwm_max()
        self.reader_thread = threading.Thread(target=self._read_loop, daemon=True, name="LidarReaderThread")
    ...
    def _read_loop(self):
        ...
        while self.running:
            ...
            packet_count += 1
            if self.debug and time.time() - self.last_debug_time > 1:
                print(f"[LIDAR] {packet_count} trames valides traitées")
                self.last_debug_time = time.time()
