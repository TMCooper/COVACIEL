import serial
import threading
import numpy as np
import matplotlib.pyplot as plt
import matplotlib.animation as animation
from matplotlib.table import Table
from collections import namedtuple

LidarPoint = namedtuple("LidarPoint", ["angle", "distance", "confidence", "timestamp"])

PACKET_LEN = 47
NUM_POINTS = 12
CONFIDENCE_THRESHOLD = 50

CRC_TABLE = [
    
] + [0] * (256 - 32)

class LidarKit:
    def __init__(self, port="/dev/ttyS0", baudrate=230400, debug=False):
        self.port = port
        self.baudrate = baudrate
        self.debug = debug
        self.ser = None
        self.running = False
        self.thread = None
        self.points = []
        self.angle_distance_map = [None] * 361
        self.lock = threading.Lock()

    def open(self):
        self.ser = serial.Serial(self.port, self.baudrate, timeout=0.1)

    def close(self):
        if self.ser and self.ser.is_open:
            self.ser.close()

    def calc_crc8(self, data):
        crc = 0
        for b in data[:46]:
            crc = CRC_TABLE[(crc ^ b) & 0xFF]
        return crc

    def start(self):
        if self.running:
            return False
        self.running = True
        self.open()
        self.thread = threading.Thread(target=self.read_loop)
        self.thread.start()
        return True

    def stop(self):
        self.running = False
        if self.thread:
            self.thread.join()
        self.close()

    def get_points(self):
        with self.lock:
            pts = self.points.copy()
            self.points.clear()
        return pts

    def get_angle_distance_map(self):
        with self.lock:
            return self.angle_distance_map.copy()

    def read_loop(self):
        while self.running:
            while self.running:
                byte = self.ser.read(1)
                if byte and byte[0] == 0x54:
                    break

            packet = bytearray()
            packet.append(0x54)
            while self.running and len(packet) < PACKET_LEN:
                more = self.ser.read(PACKET_LEN - len(packet))
                packet.extend(more)

            if len(packet) < PACKET_LEN:
                continue

            if self.calc_crc8(packet) != packet[46]:
                if self.debug:
                    print("Bad checksum, skipping...")
                continue

            start_angle = int.from_bytes(packet[4:6], "little") / 100.0
            end_angle = int.from_bytes(packet[42:44], "little") / 100.0
            timestamp = int.from_bytes(packet[44:46], "little")

            step = (end_angle - start_angle + 360.0 if end_angle < start_angle else end_angle - start_angle) / (NUM_POINTS - 1)
            new_points = []

            with self.lock:
                for i in range(NUM_POINTS):
                    j = 6 + 3 * i
                    dist = int.from_bytes(packet[j:j+2], "little") / 10.0
                    conf = packet[j+2]
                    angle = (start_angle + i * step) % 360

                    if conf >= CONFIDENCE_THRESHOLD:
                        int_angle = int(round(angle)) % 360
                        self.angle_distance_map[int_angle] = dist
                        new_points.append(LidarPoint(angle, dist / 100.0, conf, timestamp))

                self.points.extend(new_points)

class LidarMap:
    def __init__(self, lidar: LidarKit, max_range=5.0):
        self.lidar = lidar
        self.max_range = max_range

        self.fig = plt.figure(figsize=(12, 8))
        self.ax_cartesian = self.fig.add_subplot(2, 1, 1)
        self.ax_table = self.fig.add_subplot(2, 1, 2)
        self.ax_table.axis('off')

        # Vue cartésienne
        self.sc = self.ax_cartesian.scatter([], [], s=10, c='blue', alpha=0.7)
        self.ax_cartesian.set_xlim(-max_range, max_range)
        self.ax_cartesian.set_ylim(-max_range, max_range)
        self.ax_cartesian.set_xlabel("X (m)")
        self.ax_cartesian.set_ylabel("Y (m)")
        self.ax_cartesian.set_title("Carte LIDAR LD06 - Vue cartésienne")
        self.ax_cartesian.grid(True)
        self.ax_cartesian.set_aspect('equal')

    def update(self, frame):
        points = self.lidar.get_points()

        if points:
            xs = [p.distance * np.cos(np.radians(p.angle)) for p in points]
            ys = [p.distance * np.sin(np.radians(p.angle)) for p in points]
            self.sc.set_offsets(np.c_[xs, ys])

        angle_distance_map = self.lidar.get_angle_distance_map()

        # Réinitialise la table
        self.ax_table.clear()
        self.ax_table.axis('off')
        table = Table(self.ax_table, bbox=[0, 0, 1, 1])

        cell_width = 1 / 20
        cell_height = 0.1
        angles_per_row = 20
        for i in range(0, 361):
            row = i // angles_per_row
            col = i % angles_per_row
            dist = angle_distance_map[i]
            value = f"{int(dist)} cm" if dist else "-"
            table.add_cell(row, col, cell_width, cell_height, text=f"{i}°: {value}", loc='center')

        self.ax_table.add_table(table)

        return self.sc,

    def run(self):
        ani = animation.FuncAnimation(self.fig, self.update, interval=200, blit=False)
        plt.tight_layout()
        plt.show()

if __name__ == "__main__":
    lidar = LidarKit("/dev/ttyS0", debug=False)
    lidar.start()

    try:
        map_display = LidarMap(lidar)
        map_display.run()
    except KeyboardInterrupt:
        print("Fermeture par l'utilisateur...")
    finally:
        lidar.stop()
