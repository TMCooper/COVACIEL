...
class Pilote():
    ...
    async def Asserv():
        ...
        reel_speed = await Pilote.GetFourche() # Recupère la vitesse reel de la voiture
        Pilote.CalcPID(Control_car_input, reel_speed) # Ajout méthode async pour verifier si la vitesse moteur est bien égale a la consigne
        # Ajouter la methode d'ajustement de la vitesse et peut être aussi un potentiel ajustement de la valeur renvoie par CalcPID pour le transformer en ms ou en valeur entre -1 et 1 
    ...
    async def GetFourche(self):
        print(gpio.input(self.branch_fourche))
    
    async def CalcPID(self, consigne, reel_speed):
        global e_prev
        global integral
        t = 20e-3
        ...
        # Calcule P
        gain = 10 # Kp
        e = consigne - reel_speed # Consigne moins sortie
